...
def create_chessboard(path, image, pattern, gridSize, ext, overwrite=True):
    ...
    template = {
        'keypoints3d': keypoints3d.tolist(), 'keypoints2d': keypoints2d.tolist(), 'pattern':
        pattern, 'grid_size': gridSize, 'visited': False
    }
...
def detect_chessboard(path, image, out, pattern, gridSize, args):
    ...
    for i in range(args.mp):
        ...
        thread = threading.Thread(
            target=_detect_chessboard, args=(datas, path, image, out, pattern)
        )    # 应该不存在任何数据竞争
        thread.start()
    ...
    while len(proposals) > 0:
        ...
        print(
            '[detect] {} {:4.1f}% Check [{:5d}, {:5d}]'.format(
                sub,
                visited.sum() / visited.shape[0] * 100, left, right
            ),
            end=' '
        )
...
def check_chessboard(path, out):
    ...
    mywarn(
        f'python3 apps/annotation/annot_calib.py {path} --mode chessboard --annot chessboard --sub {" ".join(subs_notvalid)}'
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--grid', type=float, default=0.1, help='The length of the grid size (unit: meter)'
    )
    ...
    if args.seq:
        detect_chessboard_sequence(
            args.path, args.image, args.out, pattern=args.pattern, gridSize=args.grid, args=args
        )
    else:
        detect_chessboard(
            args.path, args.image, args.out, pattern=args.pattern, gridSize=args.grid, args=args
