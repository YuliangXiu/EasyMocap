...
from easymocap.annotator import AnnotBase, ImageFolder, vis_point
...
from easymocap.annotator.chessboard import (
    colors_chessboard_bar,
    create_chessboard,
    get_lines_chessboard,
)
from easymocap.annotator.vanish_callback import calc_vanishpoint
from easymocap.mytools import plot_cross, plot_line, read_json
...
class Matcher:
    def __init__(self, path, mode, args) -> None:
        if mode == 'chessboard':
            ...
            if 'lines_color' in annots.keys():
                ...
                lines_color = [
                    colors_chessboard_bar[i % len(colors_chessboard_bar)]
    ...
    def vis(self, img, annots, **kwargs):
        ...
        width = 5
        cv2.putText(
            img, 'Current {}: {:.0f}'.format(self.cnt, annots['keypoints2d'][self.cnt][2]),
            (border, border), cv2.FONT_HERSHEY_SIMPLEX, text_size, (0, 0, 255), width
        )
        ...
        for i, (x, y, c) in enumerate(k2d):
            if c > 0:
                plot_cross(
                    img, x, y, self.lines_color[min(len(self.lines) - 1, i)], width=width, lw=lw
                )
                plot_point(
                    img, x, y, r=lw * 2, col=self.lines_color[min(len(self.lines) - 1, i)], pid=i
                )
            if i == self.cnt:
    ...
    def detect_charuco(self, crop, annots):
        cfg = {
            'long': 6, 'short': 4, 'squareLength': 0.128, 'aruco_len': 0.1, 'aruco_type': '4X4_50'
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--pattern',
        type=lambda x: (int(x.split(',')[0]), int(x.split(',')[1])),
        help='The pattern of the chessboard',
        default=(9, 6)
    )
