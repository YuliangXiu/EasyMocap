...
def extract_images(path, ffmpeg, image):
    videos = sorted(sum([glob(join(path, 'videos', '*' + ext)) for ext in extensions], []))
    for videoname in videos:
        ...
        if os.path.exists(outpath) and (
            len(os.listdir(outpath)) > 10 or len(os.listdir(outpath)) == args.num
        ...
        if args.scale != 1 and args.transpose != -1:
            other_cmd += ' -vf "transpose={transpose},scale=iw/{scale}:ih/{scale}"'.format(
                scale=args.scale, transpose=args.transpose
            )
        elif args.scale != 1:
