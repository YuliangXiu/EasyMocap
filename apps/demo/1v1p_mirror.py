...
def demo_1v1p1f_smpl_mirror(path, body_model, spin_model, args):
    ...
    for nf in tqdm(range(start, end, args.step), desc='Optimizing'):
        ...
        for i, annot in enumerate(annots):
            ...
            result = init_with_spin(
                body_model, spin_model, image, annot['bbox'], annot['keypoints'], camera
            )
        ...
        if args.normal:
            ...
            normal = None
        body_params = multi_stage_optimize(
            body_model,
            body_params,
            bboxes,
            keypoints2d,
        ...
        if args.vis_smpl:
            ...
            render_data = {
                pids[i]: {
                    'vertices': vertices[i], 'faces': body_model.faces, 'vid': 0, 'name':
                    'human_{}'.format(pids[i])
                }
                for i in range(len(pids))
            }
...
def demo_1v1pmf_smpl_mirror(path, body_model, spin_model, args):
    ...
    for sub in subs:
        ...
        if args.normal:
            ...
            normal = None
        body_params = multi_stage_optimize(
            body_model, body_params, bboxes, keypoints2d, Pall=P, normal=normal, args=args
        )
        # write
        vertices = body_model(return_verts=True, return_tensor=False, **body_params)
        keypoints = body_model(return_verts=False, return_tensor=False, **body_params)
        ...
        for nf in tqdm(frames, desc='rendering'):
            idx = nf - start
            write_data = [{'id': pids[i], 'keypoints3d': keypoints[i * nFrames + idx]}
                          for i in range(len(pids))]
            dataset.write_keypoints3d(write_data, nf)
            for i in range(len(pids)):
            ...
            if args.vis_smpl:
                ...
                render_data = {
                    pids[i]: {
                        'vertices': vertices[i * nFrames + idx], 'faces': body_model.faces, 'vid':
                        0, 'name': 'human_{}'.format(pids[i])
                    }
                    for i in range(len(pids))
                }
                dataset.vis_smpl(render_data, image, camera, nf)
...
if __name__ == "__main__":
    ...
    parser = load_parser()
    parser.add_argument('--skel', type=str, default=None, help='path to keypoints3d')
    parser.add_argument('--direct', action='store_true')
    parser.add_argument('--video', action='store_true')
    parser.add_argument('--gtK', action='store_true')
    parser.add_argument('--normal', action='store_true', help='set to use the normal of the mirror')
