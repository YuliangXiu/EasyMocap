...
def mvposev1(dataset, args, cfg):
    ...
    affinity_model = ComposedAffinity(
        cameras=dataset.cameras, basenames=dataset.cams, cfg=cfg.affinity
    )
    ...
    for nf in tqdm(range(start, end), desc='reconstruction'):
        ...
        with Timer('associate', not args.time):
            group = simple_associate(
                annots, affinity, dimGroups, dataset.Pall, group, cfg=cfg.associate
            )
            results = group
        if args.vis_match:
...
if __name__ == "__main__":
  ...
  Demo code for multiple views and multiple persons:
    ...
    - Output: {}
    - Body  : {}
""".format(args.path, ', '.join(args.sub), args.out, args.body)
    print(help)
