...
class SMPLControl(BaseWindow):
    def __init__(self, cfg_model, opts_model, out) -> None:
        ...
        joints = self.body_model(
            return_verts=False, return_tensor=False, return_smpl_joints=True, **self._params
        )[0]
    ...
    def update_smpl(self):
        ...
        for nj in range(joints.shape[0]):
            break
            sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.03, resolution=20)
            sphere.translate(joints[nj])
    ...
    def import_params(self, ):
        ...
        dlg.add_filter(".json", "SMPL parameters")
    ...
    def add_control_smpl(self, panel, em):
        ...
        self.slider_dict = {}
        for key in ['Rh', 'Th']:
            col = gui.CollapsableVert(key, 0.33 * em, gui.Margins(em, 0, 0, 0))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='config/model/smpl_neutral.yml')
    parser.add_argument('--opts', type=str, default=[], nargs="+")

    parser.add_argument('--key', type=str, default='poses')
