...
with pytest.suppress(ImportError):
    ...
    ref = np.array([[0., 3, 0, 0, 0, 11], [22, 0, 0, 0, 17, 11], [7, 5, 0, 1, 0, 11],
                    [0, 0, 0, 0, 0, 11], [0, 0, 14, 0, 8, 11]])
...
def test_partially_fixed():
    ...
    functions = [
        m.partial_copy_four_rm_r, m.partial_copy_four_rm_c, m.partial_copy_four_cm_r,
...
def test_nocopy_wrapper():
    ...
    assert (
        'get_elem_nocopy(): incompatible function arguments.' in str(excinfo.value) and
    ...
    assert (
        'get_elem_rm_nocopy(): incompatible function arguments.' in str(excinfo.value) and
