...
def simple_dtype():
    ...
    return np.dtype({
        'names': ['bool_', 'uint_', 'float_',
                  'ldbl_'], 'formats': ['?', 'u4', 'f4', 'f{}'.format(ld.itemsize)], 'offsets':
        [0, 4, 8, (16 if ld.alignment > 4 else 12)]
    })
...
def partial_ld_offset():
    return 12 + 4 * (np.dtype('uint64').alignment
                     > 4) + 8 + 8 * (np.dtype('longdouble').alignment > 8)
...
def test_dtype(simple_dtype):
    ...
    assert m.print_dtypes() == [
        ...
        ).format(e=e), "[('e1', '" + e + "i8'), ('e2', 'u1')]",
        "[('x', 'i1'), ('y', '" + e + "u8')]", "[('cflt', '" + e + "c8'), ('cdbl', '" + e + "c16')]"
    ]

    d1 = np.dtype({
        'names': ['a', 'b'], 'formats': ['int32', 'float64'], 'offsets': [1, 10], 'itemsize': 20
    })
...
def test_recarray(simple_dtype, packed_dtype):
    ...
    assert arr.dtype == nested_dtype
    assert_equal(
        arr, [((False, 0, 0.0, -0.0), (True, 1, 1.5, -2.5)),
              ((True, 1, 1.5, -2.5), (False, 2, 3.0, -5.0)),
              ((False, 2, 3.0, -5.0), (True, 3, 4.5, -7.5))], nested_dtype
    )
    assert m.print_rec_nested(arr) == [
        "n:a=s:0,0,0,-0;b=p:1,1,1.5,-2.5", "n:a=s:1,1,1.5,-2.5;b=p:0,2,3,-5",
...
def test_array_array():
    ...
    assert arr['a'].tolist() == [[b'ABCD', b'KLMN', b'UVWX'], [b'WXYZ', b'GHIJ', b'QRST'],
...
def test_complex_array():
    ...
    assert m.print_complex_array(arr) == [
        "c:(0,0.25),(0.5,0.75)", "c:(1,1.25),(1.5,1.75)", "c:(2,2.25),(2.5,2.75)"
    ]
