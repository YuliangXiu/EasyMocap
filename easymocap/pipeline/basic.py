...
from ..pyfitting import optimizePose2D, optimizePose3D, optimizeShape
...
def smpl_from_keypoints3d2d(
    body_model, kp3ds, kp2ds, bboxes, Pall, config, args, weight_shape=None, weight_pose=None
):
    ...
    if model_type in ['smpl', 'smplh', 'smplx']:
        ...
        params_shape = optimizeShape(
            body_model, params_init, kp3ds, weight_loss=weight_shape, kintree=config['kintree']
        )
    # optimize 3D pose
    cfg = Config(args)
    cfg.device = body_model.device
...
def smpl_from_keypoints3d(body_model, kp3ds, config, args, weight_shape=None, weight_pose=None):
    model_type = body_model.model_type
    params_init = body_model.init_params(nFrames=1)
    if weight_shape is None:
        weight_shape = load_weight_shape(model_type, args.opts)
