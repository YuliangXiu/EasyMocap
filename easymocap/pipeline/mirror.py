...
from ..pyfitting import optimizeMirrorDirect, optimizeMirrorSoft
...
def load_weight_mirror(model, opts):
    if model == 'smpl':
        weight = {
            'k2d': 2e-4, 'init_poses': 1e-3, 'init_shapes': 1e-2, 'smooth_body': 5e-1,
            'smooth_poses': 1e-1, 'par_self': 5e-2, 'ver_self': 2e-2, 'par_mirror': 5e-2
        }
    elif model == 'smplh':
        weight = {
            'repro': 1,
...
def multi_stage_optimize(body_model, body_params, bboxes, keypoints2d, Pall, normal, args):
    ...
    with Timer('Optimize 2D Pose/{} frames'.format(keypoints2d.shape[1]), not args.verbose):
        if args.direct:
            ...
            body_params = optimizeMirrorDirect(
                body_model, body_params, bboxes, keypoints2d, Pall, normal, weight, config
            )
            ...
            config.OPT_POSE = False
            body_params = optimizeMirrorSoft(
                body_model, body_params, bboxes, keypoints2d, Pall, normal, weight, config
            )
            config.OPT_POSE = True
            body_params = optimizeMirrorSoft(
