...
class BaseWindow:    # this window is the basic of Open3D new render style
    ...
    def __init__(self, out, panel_rate=0.2) -> None:
        ...
        self.window = gui.Application.instance.create_window("EasyMocap Visualization", 1920, 1080)
    ...
    def _on_layout(self, layout_context):
        ...
        self.scene.frame = gui.Rect(
            contentRect.x, contentRect.y, contentRect.width - panel_width, contentRect.height
        )
        self.panel.frame = gui.Rect(
            self.scene.frame.get_right(), contentRect.y, panel_width, contentRect.height
        )
    ...
    def add_scene(self, window):
        ...
        if False:
            ...
            K = np.array([1000., 0., 500., 0., 1000., 500., 0., 0., 1.]).reshape(3, 3)
    ...
    def add_panel(self, window, em):
        panel = gui.Vert(20, gui.Margins(0.5 * em, 0.5 * em, 0.5 * em, 0.5 * em))
    ...
    def add_geometry(self, name, geom, mat=None):
        if name in self.factory.keys():
            self.scene.scene.remove_geometry(name)
        self.factory[name] = {'geom': geom, 'mat': mat}
    ...
    def add_chessboard_widget(self, param):
        chessboard = gui.CollapsableVert("Chessboard setting", 10, gui.Margins(self.em, 0, 0, 0))

        grid = gui.VGrid(2, 10)
        label0, widget0 = self.add_color('black', param['black'], self._on_chess_col_0)
        label1, widget1 = self.add_color('white', param['white'], self._on_chess_col_1)
        ...
        label_range, widget_range = self.add_vec3d(
            'ranges', [param['xrange'], param['yrange'], param['step']], self._on_chess_range
        )
        grid.add_child(label_range)
        grid.add_child(widget_range)
        # 增加棋盘格的中心选项
        label_center, widget_center = self.add_vec3d(
            'center', param['center'], self._on_chess_center
        )
