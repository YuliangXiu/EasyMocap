...
from ..mytools.camera_utils import Undistort, read_cameras, unproj
from ..mytools.debug_utils import log, myerror, mywarn
from ..mytools.file_utils import read_json, save_json, write_keypoints3d
...
def get_allname(root0, subs, ranges, root, ext, **kwargs):
    ...
    for sub in subs:
        ...
        if len(imgnames) < _ranges[1]:
            raise ValueError(
                'The number of images in {} is less than the range: {} vs {}'.format(
                    join(root0, root, sub), len(imgnames), _ranges[1]
                )
            )
...
def add_logo(img, logo_size=0.1):
    ...
    logo_ = cv2.resize(
        logo, (int(logo.shape[1] * scale), int(logo.shape[0] * scale)),
    ...
    def annots_to_numpy(annots, filter):
        ...
        for data in annots['annots']:
            ...
            if 'bound' in filter.keys():
                ...
                kpts = data['keypoints']
                valid = (kpts[:, 0] > thres *
                         width) & (kpts[:, 0] < (1 - thres) *
                                   width) & (kpts[:, 1] > thres *
                                             height) & (kpts[:, 1] < (1 - thres) * height)
                kpts[~valid] = 0
            ...
            if 'min_joint' in filter.keys():
                ...
                if valid.sum() < filter['min_joint']:
                    continue
            if 'coco17tobody25' in filter.keys() and 'keypoints' in data.keys(
            ) and data['keypoints'].shape[0] == 17:
        ...
        for key in keys:
            ...
            for d in data_all:
                if key not in d:
                    ...
                    if self.loadmp:
                        if key not in self.cache_shape.keys():
                            if key.endswith('_distort') or key.endswith('_unproj'):
                                self.cache_shape[key] = self.cache_shape[key.replace(
                                    '_distort', ''
                                ).replace('_unproj', '')]
                            else:
                    ...
                    if key not in self.cache_shape.keys():
                        if key in [
                            'dist', 'K', 'RT', 'Tc', 'Rc', 'annname', 'imgname', 'KRT', 'pid',
                            'annots'
                        ]:
                            continue
...
class Base(BaseData):
  ...
  - in {}\n  - views: {}\n  - ranges: {}'''.format(
            self.__class__.__name__, len(self), self.root, self.subs, self.ranges
        )
    ...
    def __getitem__(self, index):
        ...
        ret = {'imgname': imgname, 'annname': annname, 'annots': annots}
    ...
    def vis_body(self, body_model, params, img, camera, scale=1, mode='image'):
        ...
        for param in params:
            ...
            meshes[param.id + 1] = {
                'vertices': vertices, 'faces': body_model.faces, 'id': param.id, 'name':
                'human_{}'.format(param.id)
            }
    ...
    def write(self, body_model, body_params, data, cameras):
        ...
        for index, annname in enumerate(annnames):
            ...
            if 'render' in self.writer.keys() and self.writer.render.enable:
                ...
                vis_mesh = self.vis_body(
                    body_model,
                    params,
                    img,
                    camera,
                    scale=self.writer.render.scale,
                    mode=self.writer.render.mode
                )
                meshname = join(self.out, self.writer['render']['root'], *
                                splitname) + self.writer['render']['ext']
...
class ImageFolder(Base):
    def __init__(self, keys, pid=0, loadmp=False, compose_mp=False, **kwargs):
        ...
        self.bboxmap = {
            'keypoints2d': 'bbox', 'keypoints': 'bbox', 'handl2d': 'bbox_handl2d', 'handr2d':
            'bbox_handr2d', 'face2d': 'bbox_face2d'
        }
    ...
    def __getitem__(self, index, K=None, dist=None):
        ...
        for key, keyname in self.keys.items():
            if not self.read_flag[key]:
                continue
            if len(data['annots']['annots']) < 1:
                mywarn("no annotations in {}".format(data['imgname']))
            if index == 0 and len(data['annots']['annots']
                                 ) >= 1 and keyname not in data['annots']['annots'][0].keys():
                ...
                for i, data_ in enumerate(data['annots']['annots']):
                    if keyname not in data_.keys():
                        if key not in self.cache_shape.keys() and self.read_flag[keyname]:
                            cache_shape = {
                                'handl2d': np.zeros((21, 3)), 'handr2d': np.zeros((21, 3)),
        ...
        if not self.loadmp:
            if 'depth' in self.reader.keys():
                depthname = join(
                    self.root, self.reader['depth']['root'],
                    self.get_view(index)[1],
                    '{}.png'.format(os.path.basename(data['annname']).replace('.json', ''))
                )
...
class MultiView(ImageFolder):
        ...
        for nv in range(nViews):
            ...
            for i3d, res3d in enumerate(res3ds):
                ...
                for key in ['keypoints3d', 'handl3d', 'handr3d', 'face3d']:
                    if key not in res3d.keys():
                        continue
                    k3d = res3d[key]
                    k3d0 = np.ascontiguousarray(k3d[:, :3])
                    points2d_repro, _ = cv2.projectPoints(k3d0, Rvec, T, K, dist)
    ...
    def write_keypoints3d(self, results, data):
        ...
        write_keypoints3d(outname, write_, keys=['keypoints3d', 'handl3d', 'handr3d', 'face3d'])
    ...
    def write_all(self, results, data):
        ...
        outname = join(self.out, self.writer.keypoints3d.root, basename)
        write_keypoints3d(
            outname, results_all, keys=['keypoints3d', 'handl3d', 'handr3d', 'face3d']
        )
        ...
        for nv, annname in enumerate(annnames):
            ...
            for res_ in results:
                ...
                for key in ['keypoints', 'handl2d', 'handr2d', 'face2d']:
                    if key not in data.keys() or key not in res_.keys():
                        continue
                    if len(res_[key][nv]) == 1:
                        ...
                        res2d[self.bboxmap[key]] = [
                            k2d[0, 0] - scale, k2d[1, 0] - scale, k2d[0, 0] + scale,
                            k2d[1, 0] + scale, 1.
                        ]
                    else:
            ...
            outname = join(
                self.out, self.writer.keypoints2d.root, os.sep.join(annname.split(os.sep)[-2:])
            )
            ...
            if key == 'keypoints3d':
                ...
                for sub in self.subs_vis:
                    ...
                    if params.crop and 'bbox' in annots_match[nv]['annots'][0].keys():
                        bbox = smooth_bbox(
                            annots_match[nv]['annots'][0]['bbox'],
                            name='{}_{}_body'.format(key, nv)
                        )
                        vis = crop_image(vis, bbox)
                    visall.append(vis)
                visall = merge(visall, resize=False)
    ...
    def write(self, body_model, body_params, data, cameras):
        ...
        for index in iterator:
            ...
            if self.loadmp:
                ...
                if params.poses.shape[0] == 1:
                    ...
                    for nv, sub in enumerate(self.subs):
                        ...
                        fullname = join(
                            self.out, self.writer.fullpose.root, sub, basename + '.json'
                        )
            ...
            outname = join(self.out, self.writer['render']['root'],
                           basename) + self.writer['render']['ext']
...
class Keypoints3D(BaseData):
    ...
    def write(self, body_model, body_params, data):
        config = {
            'align_first': True, 'vis_skel': True, 'vis_size': 1024, 'z_offset': 1.5, 'x_offset':
            0.15, 'rot_axis': [0., 1., 0.], 'rot_angle': np.pi / 2
        }
        if config['vis_skel']:
...
class MultiViewSocket(BaseData):
    ...
    def __getitem__(self, index):
        ...
        for nv, sub in enumerate(self.subs):
            ...
            ret = {
                'imgname': 'images/{}/{:06d}.jpg'.format(sub, frame), 'annname':
                'annots/{}/{:06d}.jpg'.format(sub, frame), 'annots': annots
            }
            if False:
...
class MV1Psocket(MultiViewSocket):
    ...
    def vis_keypoints2d_mv(self, images, detections, outname=None, vis_id=True):
        return super().vis_keypoints2d_mv(
            images, {self.pid: detections}, outname=outname, vis_id=vis_id, use_limb_color=True
        )
