...
class AnnotBase:
    def __init__(
        ...
        self.param = {
            'frame': 0, 'nFrames': self.nFrames, 'kpts_name': kpts_name, 'bbox_name': bbox_name,
            'select': {bbox_name: -1, 'corner':
                       -1}, 'click': None, 'name': name, 'body': body, 'capture_screen': False
        }
    ...
    def set_param(param, imgname, annname, nf, no_img=False):
        ...
        if not no_img:
            ...
            param['scale'] = min(
                CV_KEY.WINDOW_HEIGHT / img0.shape[0], CV_KEY.WINDOW_WIDTH / img0.shape[1]
            )
            # param['scale'] = 1
...
class AnnotMV:
    def __init__(
        ...
        self.register_keys = {
            'Q': register_keys['q'], 'h': register_keys['H'], 'A': register_keys['A']
        }
        self.register_keys.update(key_funcs)
...
class AnnotMVMerge(BaseWindow):
    ...
    def __init__(self, datasets, register_keys, vis_funcs, vis_funcs_view, callbacks, body) -> None:
        ...
        self.params_view = {
            sub: {
                'body': body, 'bbox_name': bbox_name, 'kpts_name': kpts_name, 'select':
                {bbox_name: -1}
            }
            for sub in self.subs
        }
        ...
        self.param = {
            'scale': scale, 'ranges': ranges, 'click': None, 'start': None, 'end': None, 'frames':
            frames, 'body': body, 'bbox_name': bbox_name, 'kpts_name': kpts_name, 'select':
            {'camera': -1, bbox_name: -1, 'corner': -1}
        }
    ...
    def get_move(wasd):
        get_frame = {
            'a': lambda x, f: f - 1, 'd': lambda x, f: f + 1, 'w': lambda x, f: f - x.step, 's':
            lambda x, f: f + x.step
        }[wasd]
        text = {
            'a': 'Move to last frame', 'd': 'Move to next frame', 'w': 'Move to last step frame',
    ...
    def run(self, key=None, noshow=False):
        ...
        for nv, img in enumerate(imgs):
            for func in self.vis_funcs_view:
                img = func(
                    img,
                    self.param['annots'][nv],
                    name=self.subs[nv],
                    **self.params_view[self.subs[nv]]
                )
            if nv == actv:
                cv2.rectangle(
                    img, (0, 0), (img.shape[1], img.shape[0]), (0, 0, 255), img.shape[1] // 100
                )
            imgs[nv] = img
        img = merge(imgs)
        for func in self.vis_funcs:
...
class AnnotMVMain:
    def __init__(
        ...
        self.scale = scale
        self.param = {
            'scale': scale, 'ranges': ranges, 'click': None, 'start': None, 'end': None, 'select':
            {'camera': -1}
        }
...
def load_parser():
    ...
    parser.add_argument(
        '--sub', type=str, nargs='+', default=[], help='the sub folder lists when in video mode'
    )
...
def parse_parser(parser):
    ...
    if args.from_file is not None and args.from_file.endswith('.txt'):
        ...
        if not os.path.exists(join(args.path, args.image)):
            print(
                '{} not exists, Please run extract_image first'.format(join(args.path, args.image))
            )
            raise FileNotFoundError
        subs = sorted(os.listdir(join(args.path, args.image)))
        subs = [
            s for s in subs
            if os.path.isdir(join(args.path, args.image, s)) and not s.startswith('._')
        ]
