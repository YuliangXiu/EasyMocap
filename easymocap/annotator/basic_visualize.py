...
def vis_line(img, start, end, **kwargs):
    if start is not None and end is not None:
        ...
        cv2.line(img, (int(start[0]), int(start[1])), (int(end[0]), int(end[1])), (0, 255, 0), lw)
    return img
...
def vis_bbox(img, start, end, **kwargs):
    if start is not None and end is not None:
        lw = max(2, img.shape[0] // 500)
        cv2.rectangle(
            img, (int(start[0]), int(start[1])), (int(end[0]), int(end[1])), (0, 255, 0), lw
        )
    return img
...
def plot_text(img, annots, imgname, **kwargs):
    if 'isKeyframe' in annots.keys():
        if annots['isKeyframe']:    # 关键帧使用红框表示
            ...
            cv2.rectangle(
                img, (0, 0), (img.shape[1], img.shape[0]), (0, 255, 0), img.shape[1] // 100
            )
    ...
    width = 2 * text_size
    cv2.putText(
        img, '{}'.format(imgname), (border, img.shape[0] - border), cv2.FONT_HERSHEY_SIMPLEX,
        text_size, (0, 0, 255), width
    )
    # 显示标注进度条:
...
def plot_bbox_body(img, annots, **kwargs):
    ...
    for data in annots:
        ...
        w = 10 * lw
        cv2.rectangle(
            img, (int((x1 + x2) / 2 - w), int((y1 + y2) / 2 - w * ratio)),
            (int((x1 + x2) / 2 + w), int((y1 + y2) / 2 + w * ratio)), color, -1
        )
        cv2.putText(
            img, '{}'.format(pid), (int(x1), int(y1) + 20), cv2.FONT_HERSHEY_SIMPLEX, 5, color, 2
        )
    return img
...
def plot_bbox_sp(img, annots, bbox_type='handl_bbox', add_center=False):
    ...
    for data in annots['annots']:
        ...
        cv2.putText(
            img, '{}'.format(pid), (int(x1), int(y1) + 20), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2
        )
    return img
...
def plot_skeleton(img, annots, body='body25', bbox_name='bbox', kpts_name='keypoints', **kwargs):
    ...
    for data in annots:
        ...
        if kpts_name in data.keys():
            ...
            plot_keypoints(
                img, keypoints, pid, CONFIG[body], vis_conf=vis_conf, use_limb_color=True
            )
...
def vis_active_bbox(img, annots, select, bbox_name, **kwargs):
    ...
    if active == -1 or active >= len(annots['annots']):
        ...
        cv2.rectangle(
            mask, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), get_rgb(pid), -1
        )
