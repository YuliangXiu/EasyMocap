...
def get_auto_track(mode='kpts'):
    ...
    def auto_track(self, param, **kwargs):
        ...
        if mode == 'kpts':
            ...
            diff = np.linalg.norm(
                keypoints_now[:, None, :, :2] - keypoints_pre[None, :, :, :2], axis=-1
            )
        ...
        if len(used_p) != len(pre_ids):
            param['stop'] = True
            print(
                '>>> Stop because missing key: {}'.format([i for i in pre_ids if i not in used_p])
            )
            print(dist)
...
def create_bbox(self, param, **kwargs):
    ...
    if len(nowids) == 0:
        ...
        maxID = max(nowids) + 1
    data = {
        'personID': maxID, bbox_name: [start[0], start[1], end[0], end[1], 1], kpts_name:
...
def create_bbox_mv(self, param, **kwargs):
    ...
    data = {
        'personID': maxID, bbox_name: [start[0], start[1], end[0], end[1], 1], kpts_name:
        [[0., 0., 0.] for _ in range(CONFIG[body]['nJoints'])]
    }
    annots.append(data)
    param['start'], param['end'] = None, None
...
def callback_select_image(click, select, ranges, **kwargs):
    ...
    res = (click[:, 0]
           > ranges[:, 0]) & (click[:, 0]
                              < ranges[:, 2]) & (click[:, 1]
                                                 > ranges[:, 1]) & (click[:, 1] < ranges[:, 3])
    if res.any():
        select['camera'] = int(np.where(res)[0])


def callback_select_image_bbox(
    click, start, end, select, ranges, annots, bbox_name='bbox', **kwargs
):
    if click is None:
        return 0
    ranges = np.array(ranges)
