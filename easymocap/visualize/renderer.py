        ...
        for nv, img_ in enumerate(images):
            ...
            for iextra, _mesh in enumerate(self.extra_mesh):
                if True:
                    ...
                    material = pyrender.MetallicRoughnessMaterial(
                        metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=(0., 0., 0., 1.)
                    )
            ...
            for trackId, data in render_data.items():
                ...
                if 'colors' not in data.keys():
                    ...
                    mesh.apply_transform(rot)
                    material = pyrender.MetallicRoughnessMaterial(
                        metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=(1., 1., 1.)
                    )
                    mesh = pyrender.Mesh.from_trimesh(mesh, material=material)
                    scene.add(mesh, data['name'])
        ...
        for nv in range(nViews):
            ...
            if camera is None:
                if K is None:
                    ...
                    camera = pyrender.camera.IntrinsicsCamera(
                        fx=K[nv][0, 0], fy=K[nv][1, 1], cx=K[nv][0, 2], cy=K[nv][1, 2]
                    )
            scene.add(camera, pose=camera_pose)
            # Alpha channel was not working previously need to check again
            # Until this is fixed use hack with depth image to get the opacity
...
def render_results(
    ...
    render0 = cv2.addWeighted(
        cv2.bitwise_and(img, 255 - imgsrender[:, :, 3:4].repeat(3, 2)), 1, imgsrender[:, :, :3], 1,
        0.0
    )
    if rotate:
        ...
        if fix_center is None:
            center = np.mean(
                np.vstack([v['vertices'] for i, v in render_data.items()]), axis=0, keepdims=True
            )
        ...
        render1 = cv2.addWeighted(
            cv2.bitwise_and(blank, 255 - imgsrender[:, :, 3:4].repeat(3, 2)), 1,
            imgsrender[:, :, :3], 1, 0.0
        )
