...
class Visualizer:
    def __init__(
        ...
        print(colored('the results are saved at {}'.format(self.data_dir), 'yellow'))
        self.H = H
        self.W = W
    ...
    def __call__(self, output, batch):
        ...
        for key in ['rgb', 'rgb_map']:
            ...
            if False:
                res[coord[:, 0], coord[:, 1]] = out_ * output['acc_map'][:, None] + res[
                    coord[:, 0], coord[:, 1]] * (1 - output['acc_map'][:, None])
        ...
        for key in ['semantic_map']:
            ...
            color = np.stack([get_rgb_01(0),
                              get_rgb_01(1),
                              get_rgb_01(2),
                              get_rgb_01(3)])[:semantic.shape[-1]]
            color[0] = 1.
