...
def sample_rays_rate(bounds, rates, back_mask, nrays=1024, **kwargs):
    ...
    if 'method' in kwargs and kwargs['method'] == 'patch':
        ...
        for n_patch in range(center.shape[0]):
            ...
            i, j = np.meshgrid(
                np.arange(x_min, x_max, dtype=coords.dtype),
...
class AABBSampler(BaseSampler):
    ...
    def mask(self, K, R, T, H, W, **kwargs):
        ...
        if (corners_3d[..., -1] < 0.).any():    # some points is behind the camera
            ...
            for lines in [[0, 1, 3, 2, 0], [4, 5, 7, 6, 5], [0, 1, 5, 4, 0], [2, 3, 7, 6, 2],
                          [0, 2, 6, 4, 0], [1, 3, 7, 5, 1]]:
...
class AABBwMask(AABBSampler):
    ...
    def mask(self, K, R, T, H, W, **kwargs):
        ...
        if self.instance is not None:
            ...
            mask_bounds[bbox[1] - padding:bbox[3] + padding,
                        bbox[0] - padding:bbox[2] + padding] = True
        ...
        return {
            'body': {'mask': instance, 'rate': rate_body}, 'bound':
            {'mask': mask_out_body, 'rate': rate_outer}
        }
...
class TwoAABBSampler(BaseSampler):
    ...
    def get_near_far(self, ray_o, ray_d, coord):
        near_inter, far_inter, mask_inter = AABBSampler.get_near_far(
            ray_o, ray_d, self.bbox_inter, coord
        )
        near_outer, far_outer, mask_outer = AABBSampler.get_near_far(
            ray_o, ray_d, self.bbox_outer, coord
        )
...
class PlaneSampler(AABBSampler):
    ...
    def __init__(self, center, radius, zranges, xybounds=None, **kwargs):
        bounds = np.array([[center[0] - radius, center[1] - radius, center[2] + zranges[0]],
                           [center[0] + radius, center[1] + radius, center[2] + zranges[1]]])
    ...
    def __call__(self, ray_o, ray_d, coord, depth=None):
        ...
        if self.xybounds is not None:
            ...
            mask_xy = (pts[:, 0] > self.xybounds[0]) & (pts[:, 1] > self.xybounds[0]) & (
                pts[:, 0] < self.xybounds[1]
            ) & (pts[:, 1] < self.xybounds[1])
            mask[mask] &= mask_xy
            near = near[mask_xy]
            far = far[mask_xy]
...
class CylinderSampler(BaseSampler):
    ...
    def __init__(self, center, split, zranges, radius=(3., 7), **cfg):
        ...
        self.center = center
        self.bounds = np.array([[
            self.center[0] - self.radius[1], self.center[1] - self.radius[1],
            self.center[2] + self.zranges[0]
        ],
                                [
                                    self.center[0] + self.radius[1], self.center[1] +
                                    self.radius[1], self.center[2] + self.zranges[1]
                                ]],
                               dtype=np.float32)
        self.feature = {'bounds': self.bounds}
    ...
    def get_near_far_cylinder(ray_o, ray_d, viewdirs, radius):
        ...
        d_0_dir = np.sqrt(
            np.maximum((ray_o[..., :2] * ray_o[..., :2]).sum(axis=-1) - d1 * d1, 1e-5)
        )
    ...
    def __call__(self, ray_o, ray_d, coord):
        near, far, mask = self.near[coord[:, 0],
                                    coord[:, 1]], self.far[coord[:, 0],
                                                           coord[:, 1]], self._mask[coord[:, 0],
                                                                                    coord[:, 1]]
...
def create_center_radius(center, radius=5., up='y', ranges=[0, 360, 36], angle_x=0, **kwargs):
    ...
    if up == 'z':
        ...
        R = np.stack([+st, zero, -ct, zero, zero - 1, zero, -ct, zero, -st], axis=-1)
    R = R.reshape(-1, 3, 3)
