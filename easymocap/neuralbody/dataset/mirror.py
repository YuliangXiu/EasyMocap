...
from .mvbase import BaseDataset, get_bounds, read_json
...
class MirrorDataset(BaseDataset):
    def get_allnames(self, root, subs, ranges, image_args):
        ...
        for nnf, nf in enumerate(trange(*ranges, desc='cache parameters')):
            ...
            vertices = read_json(
                join(root, 'output-smpl-3d', 'vertices', sub, '{:06d}.json'.format(nf))
            )
            annots = annot['annots']
            imgname = join(root, image_args.root, sub, '{:06d}{}'.format(nf, image_args.ext))
            info = {
                'imgname': imgname, 'sub': sub, 'frame': nf, 'nf': nnf, 'nv': nv, 'index': index,
                'camera': camera, 'flip_lr': False, 'pid': 0
            }
    ...
    def get_objects(self, root, info, object_keys, object_args):
        ...
        mskname = join(
            root, object_args['human_0'].args.reader.mask.root, sub,
            '{:06d}_{}.png'.format(frame, pid)
        )
        ...
        vertices_canonical = (info['params']['vertices'] -
                              info['params']['Th']) @ info['params']['R'].T.T
        obj.feature['bounds_canonical'] = get_bounds(
            vertices_canonical, delta=object_args['human_0'].args.reader.vertices.padding
        )
        return {'human_0': obj}
    ...
    def read_image(
        ...
        if info['flip_lr']:
            return super().read_image(
                imgname, image_args, info, isgray, skip_mask, mask_global='_1.png'
            )
        else:
