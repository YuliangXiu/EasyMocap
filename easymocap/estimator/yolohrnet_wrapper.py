...
def detect_frame(detector, img, pid=0, only_bbox=False):
    ...
    for i in range(len(lDetections)):
        annot = {
            'bbox': [float(d) for d in lDetections[i]['bbox']], 'personID': pid + i, 'isKeyframe':
            False
        }
        if not only_bbox:
...
def extract_bbox(image_root, annot_root, ext, **config):
    ...
    for imgname in tqdm(imgnames, desc='{:10s}'.format(os.path.basename(annot_root))):
        ...
        for i in range(len(detections)):
            annot_ = {'bbox': [float(d) for d in detections[i]], 'isKeyframe': False}
            annot_['area'] = max(
                annot_['bbox'][2] - annot_['bbox'][0], annot_['bbox'][3] - annot_['bbox'][1]
            )**2
...
def extract_yolo_hrnet(image_root, annot_root, ext, config_yolo, config_hrnet):
    ...
    for nf, imgname in enumerate(imgnames):
        ...
        for i in range(len(detections)):
            annot_ = {
                'bbox': [float(d) for d in detections[i]], 'keypoints': points2d[i], 'isKeyframe':
                False
            }
            annot_['area'] = max(
                annot_['bbox'][2] - annot_['bbox'][0], annot_['bbox'][3] - annot_['bbox'][1]
