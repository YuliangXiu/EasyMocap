...
from .wrapper_base import bbox_from_keypoints, check_result, create_annot_file
...
def extract_2d(image_root, annot_root, tmp_root, config):
    ...
    thread = Process(
        target=convert_from_openpose, args=(tmp_root, annot_root, image_root, config['ext'])
    )    # 应该不存在任何数据竞争
    thread.start()
...
def load_openpose(opname):
    ...
    for i, d in enumerate(data['people']):
        ...
        annot = {
            'bbox': bbox_from_keypoints(keypoints), 'personID': pid + i, 'keypoints':
...
def filter_feet(kpts):
    ...
    if (kpts[[13, 14, 19], -1] > 0).all():
        l_feet = ((kpts[[19, 20, 21], -1] > 0) *
                  np.linalg.norm(kpts[[19, 20, 21], :2] - kpts[14, :2], axis=-1)).max()
    ...
    if (kpts[[10, 11], -1] > 0).all():
        l_feet = ((kpts[[22, 23, 24], -1] > 0) *
                  np.linalg.norm(kpts[[22, 23, 24], :2] - kpts[11, :2], axis=-1)).max()
    ...
    def detect_foot(self, image_root, annot_root, ext):
        ...
        for imgname in tqdm(imgnames, desc='{:10s}'.format(os.path.basename(annot_root))):
            ...
            for i, annot in enumerate(annots['annots']):
                ...
                infos[base][i] = {
                    'crop_shape': crop.shape, 'start': start, 'rot': rot, 'name': f'{base}_{i}.json'
                }
                ...
                if self.hand or self.face:
                    for key in ['handl2d', 'handr2d', 'face2d']:
                        if key in annots_sub[0].keys():
                            ...
                            annots[key] = transoform_foot(
                                info['crop_shape'], info['start'], info['rot'], khand, None
                            )
                            annots['bbox_' + key] = bbox_from_keypoints(annots[key])
                if self.fullbody:
                    kpts_np = transoform_foot(
                        info['crop_shape'], info['start'], info['rot'], keypoints, None
                    )
                else:
                    kpts = annots['keypoints']
                    kpts_np = transoform_foot(
                        info['crop_shape'], info['start'], info['rot'], keypoints, kpts
                    )
