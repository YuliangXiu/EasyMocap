...
class CritNoBorder(BaseCrit):
    ...
    def __call__(self, keypoints, bbox, **kwargs) -> bool:
        ...
        if l < self.border or r > self.width - self.border:
            if self.log:
                print('[Crit2d]: {}'.format(' '.join(['%8.3f' % (i) for i in bbox])))
            if self.log:
                print('[Error] Left or right')
            dist = np.linalg.norm(
                keypoints[self.leftidx, :2] - keypoints[self.rightidx, :2], axis=1
            )
            bbox_size = b - t
            dist = dist / bbox_size
            if dist.min() < 1e-2:
...
class ComposedFilter:
    ...
    def __str__(self) -> str:
        ...
        for filt in self.filters:
            res_ = indent + indent + '{:15s}'.format(filt.__class__.__name__
                                                    ) + ': ' + str(filt) + '\n'
            res += res_
        return res
