...
class FittingMonitor:
    def __init__(
        self, ftol=1e-5, gtol=1e-6, maxiters=100, visualize=False, verbose=False, **kwargs
    ):
...
class FittingLog:
    ...
    def __init__(self, log_name, useVisdom=False):
        ...
        if useVisdom:
            ...
            self.vis = visdom.Visdom(
                env=os.path.realpath(join(os.path.dirname(log_name), '..')).replace(os.sep, '_')
            )
        elif False:
    ...
    def step(self, loss_dict, weight_loss):
        ...
        loss = {
            key: loss_dict[key].item() * weight_loss[key]
            for key in loss_dict.keys() if weight_loss[key] > 0
        }
        if self.useVisdom:
            name = list(loss.keys())
            ...
            self.vis.line(
                ...
                opts=dict(legend=name, title=self.log_name),
                update=None if x == 0 else 'append'
            )
        elif False:
            self.writer.add_scalars(
                'data/{}'.format(self.log_name), loss, self.index[self.log_name]
            )
