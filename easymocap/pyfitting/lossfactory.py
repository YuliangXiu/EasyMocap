...
from .operation import batch_rodrigues, projection
...
class LossKeypoints3D:
    ...
    def body(self, kpts_est, **kwargs):
        ...
        diff_square = (kpts_est[:, :nJoints, :3] -
    ...
    def hand(self, kpts_est, **kwargs):
        ...
        diff_square = (kpts_est[:, 25:25 + 42, :3] -
    ...
    def face(self, kpts_est, **kwargs):
        ...
        diff_square = (kpts_est[:, 25 + 42:, :3] -
                       self.keypoints3d[:, 25 + 42:, :3]) * self.conf[:, 25 + 42:]
        return self.loss(diff_square)
...
class LossSmoothPoses:
    ...
    def _poses(self, poses):
        ...
        for nv in range(self.nViews):
            poses_ = poses[
                nv * self.nFrames:(nv + 1) * self.nFrames,
            ]
            # 计算poses插值
...
class LossRepro:
    def __init__(self, bboxes, keypoints2d, cfg) -> None:
        ...
        bbox_conf = bboxes[..., 4]
        bbox_mean_axis = -1
        bbox_sizes = (bbox_sizes * bbox_conf).sum(axis=bbox_mean_axis
                                                 ) / (1e-3 + bbox_conf.sum(axis=bbox_mean_axis))
    ...
    for key in ['poses', 'Th', 'poses_hand', 'expression']:
        ...
        if k in weight_loss.keys() and weight_loss[k] > 0.:
            ...
            for span in spans:
                if key == 'poses_hand':
                    val = torch.sum((
                        body_params['poses'][span:, 66:] -
                        body_params['poses'][:nFrames - span, 66:]
                    )**2)
                else:
                    val = torch.sum((
                        body_params[key][span:, :nPoses] -
                        body_params[key][:nFrames - span, :nPoses]
                    )**2)
...
def RegularizationLoss(body_params, body_params_init, weight_loss):
    ...
    for key in ['poses', 'shapes', 'Th', 'hands', 'head', 'expression']:
        if 'init_' + key in weight_loss.keys() and weight_loss['init_' + key] > 0.:
            if key == 'poses':
                loss_dict['init_' + key] = torch.sum(
                    (body_params[key][:, :66] - body_params_init[key][:, :66])**2
                )
            elif key == 'hands':
                loss_dict['init_' + key] = torch.sum((
                    body_params['poses'][:, 66:66 + 12] - body_params_init['poses'][:, 66:66 + 12]
                )**2)
            elif key == 'head':
                loss_dict['init_' + key] = torch.sum(
                    (body_params['poses'][:, 78:78 + 9] -
                     body_params_init['poses'][:, 78:78 + 9])**2
                )
