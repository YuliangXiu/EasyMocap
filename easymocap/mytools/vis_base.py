...
def plot_point(img, x, y, r, col, pid=-1, font_scale=-1, circle_type=-1):
    ...
    if pid != -1:
        cv2.putText(
            img, '{}'.format(pid), (int(x + 0.5), int(y + 0.5)), cv2.FONT_HERSHEY_SIMPLEX,
            font_scale, col, 1
        )
...
def plot_line(img, pt1, pt2, lw, col):
    cv2.line(
        img, (int(pt1[0] + 0.5), int(pt1[1] + 0.5)), (int(pt2[0] + 0.5), int(pt2[1] + 0.5)), col, lw
    )
...
def plot_bbox(img, bbox, pid, scale=1, vis_id=True):
    ...
    if vis_id:
        ...
        cv2.putText(
            img, '{}'.format(pid), (x1, y1 + int(25 * font_scale)), cv2.FONT_HERSHEY_SIMPLEX,
            font_scale, color, 2
        )


def plot_keypoints(
    img, points, pid, config, vis_conf=False, use_limb_color=True, lw=2, fliplr=False
):
    lw = max(lw, 2)
    H, W = img.shape[:2]
    ...
    for i in range(min(len(points), config['nJoints'])):
        ...
        if c > 0.01:
            ...
            if vis_conf:
                cv2.putText(
                    img, '{:.1f}'.format(c), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, text_size,
                    col, 2
                )


def plot_keypoints_auto(
    img,
    points,
    pid,
    vis_conf=False,
    ...
    if config_name is None:
        config_name = {
            25: 'body25', 15: 'body15', 21: 'hand', 42: 'handlr', 17: 'coco', 1: 'points', 67:
            'bodyhand', 137: 'total', 79: 'up', 19: 'ochuman'
        }[len(points)]
    ...
    for ii, (i, j) in enumerate(config['kintree']):
        ...
        if pt1[-1] > 0.01 and pt2[-1] > 0.01:
            image = cv2.line(
                img, (int(pt1[0] * scale + 0.5), int(pt1[1] * scale + 0.5)),
                (int(pt2[0] * scale + 0.5), int(pt2[1] * scale + 0.5)), col, lw
            )
    lw = img.shape[0] // 200
    if config_name == 'hand':
        lw = img.shape[0] // 500
    ...
    for i in range(len(points)):
        ...
        if c > 0.01:
            ...
            if vis_conf:
                cv2.putText(
                    img, '{:.1f}'.format(c), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, col, 2
                )
...
def plot_keypoints_total(img, annots, scale, pid_offset=0):
    ...
    for annot in annots:
        ...
        for key in ['keypoints', 'handl2d', 'handr2d']:
            ...
            lw = max(lw, 1)
            plot_keypoints_auto(
                img, annot[key], pid, vis_conf=False, use_limb_color=False, scale=scale, lw=lw
            )
...
def plot_points2d(img, points2d, lines, lw=-1, col=(0, 255, 0), putText=True, style='+'):
    ...
    for i, (x, y, v) in enumerate(points2d):
        ...
        if putText:
            ...
            font_scale = img.shape[0] / 1000
            cv2.putText(
                img, '{}'.format(i), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, font_scale, c, 2
            )
    for i, j in lines:
