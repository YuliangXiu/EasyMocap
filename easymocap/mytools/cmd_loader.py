...
def load_parser():
    ...
    parser.add_argument('--pid', type=int, nargs='+', default=[0], help='the person IDs')
    parser.add_argument('--max_person', type=int, default=-1, help='maximum number of person')
    ...
    parser.add_argument(
        ...
        type=str,
        default='body25',
        choices=['body15', 'body25', 'h36m', 'bodyhand', 'bodyhandface', 'handl', 'handr', 'total']
    )
    parser.add_argument(
        '--model', type=str, default='smpl', choices=['smpl', 'smplh', 'smplx', 'manol', 'manor']
    )
    parser.add_argument(
    ...
    detec.add_argument(
        "--thres2d", type=float, default=0.3, help="The threshold for suppress noisy kpts"
    )
    #
    ...
    recon.add_argument(
        '--smooth3d', type=int, help='the size of window to smooth keypoints3d', default=0
    )
    recon.add_argument(
        '--MAX_REPRO_ERROR', type=int, help='The threshold of reprojection error', default=50
    )
    recon.add_argument(
        '--MAX_SPEED_ERROR', type=int, help='The threshold of reprojection error', default=50
    )
    ...
    output.add_argument(
        '--sub_vis', type=str, nargs='+', default=[], help='the sub folder lists for visualization'
    )
    #
    # debug
    #
    ...
    parser.add_argument(
        '--opts', help="Modify config options using the command-line", default=[], nargs='+'
    )
    parser.add_argument(
        '--cfg_opts', help="Modify config options using the command-line", default=[], nargs='+'
    )
    return parser
