...
from .debug_utils import log, mywarn
...
def robust_triangulate_point(kpts2d, Pall, dist_max, min_v=3):
    ...
    # 增加一项使用的视角数的加成
    weight_sum = (weight * valid).sum(axis=0
                                     ) + ((valid > 0) * weight_self).sum(axis=0) - min_v * 100
...
def remove_outjoint(kpts2d, Pall, out_joint, dist_max, min_view=3, debug=False):
    ...
    for nj in out_joint:
        ...
        index_j, point = robust_triangulate_point(
            kpts2d[valid, nj:nj + 1], Pall[valid], dist_max=dist_max, min_v=3
        )
...
def iterative_triangulate(
    ...
    while True:
        ...
        if len(out_view) > 1:
            ...
            if debug:
                mywarn('[triangulate] Remove outlier view: {}'.format(ratio_outlier_view))
...
class BaseTriangulator:
    ...
    def triangulate_with_results(self, pid, data, results):
        ...
        for key in self.keys:
            ...
            if len(results) == 0:
                kpts3d, kpts2d = iterative_triangulate(
                    data[key + '_unproj'], data['RT'], debug=self.debug, **self.config[key]
                )
            else:
                if len(results) == 1:
                    ...
                    vel = (previous1[:, :3] - previous0[:, :3]) * ((previous0[:, -1:] > 0) &
                                                                   (previous0[:, -1:] > 0))
...
class SimpleTriangulator(BaseTriangulator):
    def __init__(self, keys, debug, config, pid=0, disable_previous=False) -> None:
    ...
    def report(self):
        ...
        for key in self.infos[0].keys():
            ...
            values = [
                np.mean(np.stack([info[key][metric] for info in self.infos]), axis=0)
...
class SimpleMatchAndTriangulator(SimpleTriangulator):
    ...
    def distance_by_epipolar(pts0, pts1, K0, K1, R0, T0, R1, T1):
        ...
        if False:
            ...
            img4, img3 = drawlines(
                img0.copy(), img0.copy(), lines0.reshape(-1, 3), pts1.reshape(-1, 3),
                pts0.reshape(-1, 3)
            )
            img5, img6 = drawlines(
                img0.copy(), img0.copy(), lines1.reshape(-1, 3), pts0.reshape(-1, 3),
                pts1.reshape(-1, 3)
            )
        ...
        # TODO: / sqrt(A^2 + B^2)
        dist01 = np.abs(
            np.sum(lines0[:, None, :, :2] * pts1[None, :, :, :2], axis=-1) + lines0[:, None, :, 2]
        )
        conf = pts0[:, None, :, 2] * pts1[None, :, :, 2]
        dist10 = np.abs(
            np.sum(lines1[:, None, :, :2] * pts0[None, :, :, :2], axis=-1) + lines1[:, None, :, 2]
        )

        dist = np.sum(dist01 * conf + dist10.transpose(1, 0, 2) * conf,
                      axis=-1) / (conf.sum(axis=-1) + 1e-5) / 2
        return dist
    ...
    def _simple_associate2d_triangulate(self, data, affinity, dimGroups, prev_id):
        ...
        for idx in sortidx:
            ...
            if prev_id[idx] != -1:
                ...
                results = []
            proposals = check_cluster(
                affinity,
                row=idx,
                views=views,
                dimGroups=dimGroups,
                indices=idx_zero.copy(),
                p2dAssigned=p2dAssigned,
                visited=visited
            )
            for indices in proposals:
                ...
                if valid_view.sum() < self.min_views:
                    self.log(
                        '[associate] Skip proposal {}->{} with not enough valid view {}'.format(
                            idx, indices, (k2d[..., 2] > 0).sum(axis=-1)
                        )
                    )
                    continue
                ...
                if valid_joint.sum() < self.min_joints:
                    self.log(
                        '[associate] Skip proposal {}->{} as not enough joints'.format(
                            idx, indices
                        )
                    )
                    continue
                ...
                if (indices < 0).all():
                    ...
                    ipdb.set_trace()
                self.log(
                    '[associate] Add indices {}, valid {}'.format(
                        indices, (k2d[..., 2] > 0).sum(axis=-1)
                    )
                )
                res['id'] = pid
                res['indices'] = indices
            ...
            visited[idx] = 1
        self.log(
            '[associate] {} points not visited, {} not assigned'.format(
                visited.shape[0] - visited.sum(), (p2dAssigned == -1).sum()
            )
        )
        ...
        for v0 in range(nViews - 1):
            for v1 in range(1, nViews):
                ...
                if True:
                    ...
                    dist = SimpleMatchAndTriangulator.distance_by_epipolar(
                        pts0, pts1, K0, K1, R0, T0, R1, T1
                    )
    ...
    def _svt_optimize_affinity(self, affinity, dimGroups):
        ...
        aff_svt = pymatchlr.matchSVT(
            affinity, dimGroups, SimpleConstrain(dimGroups), observe, self.cfg_svt
        )
    ...
    def _track_add(self, res):
        ...
        if pid == -1:
            ...
            self.log(
                '[{:06d}] Initialize track {}, valid joints={}'.format(
                    self.time, pid, (res['keypoints3d'][:, -1] > 0.01).sum()
                )
            )
        self.tracks[pid] = {
            'start_time': self.time, 'end_time': self.time + 1, 'missing_frame': [], 'infos': [res]
        }
    ...
    def _track_update(self, res, pid):
        ...
        self.log(
            '[{:06d}] Update track {} [{}->{}], valid joints={}'.format(
                self.time, pid, info['start_time'], info['end_time'],
                (res['keypoints3d'][:, -1] > 0.1).sum()
            )
        )
    ...
    def _track_and_update(self, data, results):
        ...
        for idx_match, res in enumerate(results):
            ...
            pids_free = [
                pid for pid in self.tracks.keys() if self.tracks[pid]['end_time'] != self.time + 1
            ]
            pids_used = [
                pid for pid in self.tracks.keys() if self.tracks[pid]['end_time'] == self.time + 1
            ]
            ...
            def check_dist(k3d_check):
                ...
                dist_mean = (
                    (conf > 0.1).sum(axis=-1) < self.min_joints
                ) * cfg.track_dist_max + np.sum(dist * conf,
                                                axis=-1) / (1e-5 + np.sum(conf, axis=-1))
            ...
            if len(pids_free) > 0:
                k3d_check = np.stack([
                    self.tracks[pid]['infos'][-1]['keypoints3d'] for pid in pids_free
                ])
            ...
            if len(pids_used) > 0:
                k3d_check = np.stack([
                    self.tracks[pid]['infos'][-1]['keypoints3d'] for pid in pids_used
                ])
            ...
            if flag_tracked >= 0 and (flag_current == NOT_VISITED or flag_current == NOT_FOUND):
                ...
                # 没有跟踪到，但是有当前帧的3D的，合并
                self.log(
                    '[{:06d}] Merge track {} to {}'.format(
                        self.time, idx_match, pids_used[flag_current]
                    )
                )
...
def simple_match(data):
    ...
    dims = [d.shape[0] for d in data[key]]
    dimGroups = np.cumsum([0] + dims)
    affinity = SimpleMatchAndTriangulator.calculate_affinity_MxM(
        dims, dimGroups, data, key, DIST_MAX=0.1
    )
    import pymatchlr
    observe = np.ones_like(affinity)
    cfg_svt = {
