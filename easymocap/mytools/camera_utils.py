...
def write_intri(intri_name, cameras):
    ...
    for key_, val in cameras.items():
        ...
        assert dist.shape == (1, 5) or dist.shape == (5, 1) or dist.shape == (
            1, 4
        ) or dist.shape == (4, 1), dist.shape
...
def get_Pall(cameras, camnames):
    Pall = np.stack([
        cameras[cam]['K'] @ np.hstack((cameras[cam]['R'], cameras[cam]['T'])) for cam in camnames
    ])
    return Pall
...
def get_fundamental_matrix(cameras, basenames):
    ...
    fundamental_op = lambda K_0, R_0, T_0, K_1, R_1, T_1: np.linalg.inv(K_0).T @ (
        R_0 @ R_1.T
    ) @ K_1.T @ skew_op(K_1 @ R_1 @ R_0.T @ (T_0 - R_0 @ R_1.T @ T_1))
    fundamental_RT_op = lambda K_0, RT_0, K_1, RT_1: fundamental_op(
...
def interp_cameras(cameras, keys, step=20, loop=True, allstep=-1, **kwargs):
    ...
    for ik in range(len(keys)):
        if ik == len(keys) - 1 and not loop:
            break
        if loop:
            start, end = (ik * tall.shape[0]) // len(keys), int((ik + 1) * tall.shape[0]
                                                               ) // len(keys)
            print(ik, start, end, tall.shape)
        else:
            start, end = (ik * tall.shape[0]) // (len(keys) - 1), int((ik + 1) * tall.shape[0]
                                                                     ) // (len(keys) - 1)
