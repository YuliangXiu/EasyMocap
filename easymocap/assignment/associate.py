...
def set_keypoints2d(indices, annots, Pall, dimGroups):
    ...
    keypoints2d = np.stack([
        annots[nv][indices[nv] - dimGroups[nv]]['keypoints'].copy() for nv in Vused
    ])
...
def simple_associate(annots, affinity, dimGroups, Pall, group, cfg):
    ...
    for idx in sortidx:
        ...
        for nv in range(nViews):
            match = np.where((affinity[idx, dimGroups[nv]:dimGroups[nv + 1]] > 0.) &
        ...
        while len(proposals) > 0:
            ...
            err = ((kptsRepro[:, :, 2] * keypoints2d[:, :, 2])
                   > 0.) * np.linalg.norm(kptsRepro[:, :, :2] - keypoints2d[:, :, :2], axis=2)
            ...
            if flag:
                ...
                results.append({
                    'indices': proposal, 'keypoints2d': keypoints2d, 'keypoints3d': keypoints3d,
                    'Vused': Vused, 'error': err
                })
            else:
                # make new proposals
