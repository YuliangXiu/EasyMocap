...
from ..mytools.file_utils import (
    read_json,
    save_annot,
    save_json,
    write_keypoints3d,
...
class BaseTrack:
    def __init__(self, path, out, WINDOW_SIZE, MIN_FRAMES, SMOOTH_SIZE) -> None:
        ...
        self.svt_args = {'maxIter': 1000, 'w_sparse': 0.3, 'w_rank': 10, 'tol': 1e-4, 'log': False}
        ...
        for (nf0, ni0), (nf1, ni1) in connects:
            ...
            if id0 != -1 and id1 == -1:
                if nf1 in frames_of_id[id0].keys():
                    log(
                        f'Merge conflict1 nf0: {nf0} ni0: {ni0} id0: {id0} nf1: {nf1} ni1: {ni1} id1: {id1}'
                    )
                    continue
            ...
            if id0 == -1 and id1 != -1:
                if nf0 in frames_of_id[id1].keys():
                    log(
                        f'Merge conflict2 nf0: {nf0} ni0: {ni0} id0: {id0} nf1: {nf1} ni1: {ni1} id1: {id1}'
                    )
                    continue
...
class Track2D(BaseTrack):
    ...
    def _compute_dist(self, dimGroups, results_window):
        ...
        for i in range(window_size - 1):
            ...
            for j in range(i + 1, window_size):
                ...
                areas_pre = (bbox_pre[..., 2] -
                             bbox_pre[..., 0]) * (bbox_pre[..., 3] - bbox_pre[..., 1])
                areas_now = (bbox_now[..., 2] -
                             bbox_now[..., 0]) * (bbox_now[..., 3] - bbox_now[..., 1])
                # 左边界的大值
    ...
    def _interpolate(self, info_left, info_right, weight):
        bbox = [
            info_left['bbox'][i] * weight + info_left['bbox'][i] * (1 - weight) for i in range(5)
        ]
        ...
        for nj in range(len(kpts_l)):
            if kpts_l[nj][2] < 0.1 or kpts_r[nj][2] < 0.1:
                ...
                kpts.append([
                    kpts_l[nj][i] * weight + kpts_r[nj][i] * (1 - weight) for i in range(3)
                ])
