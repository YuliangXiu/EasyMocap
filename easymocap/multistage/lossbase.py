...
class AnyReg(LossBase):
    def __init__(
        self, key, norm, dim=-1, reduce='sum', norm_info={}, ranges=[], index=[], **kwargs
    ):
        super().__init__()
        self.ranges = ranges
        self.index = index
...
class RegPrior(AnyReg):
    ...
    def forward(self, poses, **kwargs):
        ...
        loss = 0.1 * l2loss + L2loss + 0.0005 * (exploss + nexploss) / (
            len(self.expdims) + len(self.nexpdims)
        ) + 0.01 * alll2loss
        return loss
...
class AnySmooth(LossBase):
    ...
    def check_at_start(self, **kwargs):
        ...
        contents = [
            name,
            vel.mean(axis=0).tolist(),
            vel.max(axis=0).tolist(),
            vel.argmax(axis=0).tolist()
        ]
    ...
    def check_at_end(self, **kwargs):
        ...
        contents.extend([
            err_after.mean(axis=0).tolist(),
            err_after.max(axis=0).tolist(),
            err_after.argmax(axis=0).tolist()
        ])
        print_table(header, contents)

    def __str__(self) -> str:
        return "smooth in {} frames, range={}, norm={}".format(
            self.weight, self.ranges, self.norm_name
        )
...
class LimbLength(BaseKeypoints):
    ...
    def set_gt(self, index_gt, ranges_gt):
        ...
        limb_conf = np.minimum(
            keypoints3d[..., kintree[:, 1], -1], keypoints3d[..., kintree[:, 0], -1]
        )
