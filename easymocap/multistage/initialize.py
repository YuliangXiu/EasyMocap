...
from ..mytools.debug_utils import myerror, mywarn
...
class InitRT:
    ...
    def __call__(self, body_model, body_params, infos):
        ...
        if not conf.all():
            myerror(
                "The torso in frames {} is not valid, please check the 3d keypoints".format(
                    np.where(~conf)
                )
            )
        if len(torso.shape) == 3:
            ...
            R_flat = svd_rot(
                torso_temp.reshape(-1, *torso_temp.shape[-2:]),
                torso.reshape(-1, *torso.shape[-2:])
            )
            ...
            for nper in range(Th.shape[1]):
                for nf in trange(1, Th.shape[0], desc='Check Th of {}'.format(nper)):
                    ...
                    if dist > self.T_thres:
                        mywarn(
                            '[Check Th] distance in frame {} = {} larger than {}'.format(
                                nf, dist, self.T_thres
                            )
                        )
                        Th[nf, nper] = tpre
        body_params['Th'] = Th
