...
class SMPLlayer(nn.Module):
        ...
        **kwargs
        ...
        if regressor_path is not None and use_joints:
            ...
            if self.use_pose_blending:
                j_posedirs = torch.einsum('ab, bde->ade',
        ...
        if self.model_type == 'smplh':
            ...
            for key in ['LEFT', 'RIGHT']:
                ...
                val = to_tensor(
                    to_np(data['hands_components'][:self.num_pca_comps, :]), dtype=dtype
                )
            ...
            for key in ['Ll', 'Rr']:
                ...
                self.register_buffer('mHandsMean' + key[0], val)
                val = to_tensor(
                    to_np(data['hands_components' + key[1]][:self.num_pca_comps, :]), dtype=dtype
                )
                self.register_buffer('mHandsComponents' + key[0], val)
            self.use_pca = True
    ...
    def extend_pose(self, poses):
        ...
        if self.model_type == 'mano':
            poses_hand = self.extend_hand(
                poses[..., 3:], self.use_pca, self.use_flat_mean, self.mHandsComponents,
                self.mHandsMean
            )
    ...
    def check_params(self, body_params):
        ...
        if body_params['poses'].shape[1] != self.NUM_POSES:
            body_params['poses'] = np.hstack((
                body_params['poses'],
                np.zeros((nFrames, self.NUM_POSES - body_params['poses'].shape[1]))
            ))
