    ...
    for nf, inpname in enumerate(tqdm(inplists)):
        ...
        for gt in gts:
            ...
            if gt['keypoints3d'].shape[1] == 3:
                gt['keypoints3d'] = np.hstack([
                    gt['keypoints3d'], np.ones((gt['keypoints3d'].shape[0], 1))
                ])
        for est in ests:
            est['keypoints3d'] = conversion.out(est['keypoints3d'])
            if est['keypoints3d'].shape[1] == 3:
                est['keypoints3d'] = np.hstack([
                    est['keypoints3d'],
                    np.ones((est['keypoints3d'].shape[0], 1))
                ])
        # 这一步将交换est的顺序
        if mode == 'single':
            # 单人的：直接匹配上
        ...
        for i, data in enumerate(gts):
            ...
            # 计算各种误差，存成字典
            result = keypoints_error(
                kpts_gt, kpts_est, conversion.commons, joint_level=args.joint, use_align=args.align
            )
            result['nf'] = nf
            result['id'] = data['id']
...
def run_eval_keypoints_mono(inp, out, type_i, type_o, type_e, step_gt, cam_path, mode='single'):
    ...
    if args.cam_res is not None:
        cameras_res = read_camera(
            join(args.cam_res, 'intri.yml'), join(args.cam_res, 'extri.yml'), views
        )
    ...
    for view in views:
        ...
        for outname in outlists:
            ...
            for i, data in enumerate(gts):
                ...
                # 计算各种误差，存成字典
                result = keypoints_error(
                    kpts_gt, kpts_est, conversion.commons, joint_level=args.joint, use_align=True
                )
                result['pid'] = data['id']
                result['view'] = view
                results.append(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--type_i', type=str, default='body25', help='Type of ground-truth keypoints'
    )
    parser.add_argument('--type_o', type=str, default='body25', help='Type of output keypoints')
    parser.add_argument('--type_e', type=str, default=None, help='Type of evaluation keypoints')
    parser.add_argument(
        '--mode',
        type=str,
        default='single',
        choices=['single', 'matched', 'greedy'],
    ...
    parser.add_argument('--joint', action='store_true', help='report each joint')
    parser.add_argument('--align', action='store_true', help='report each joint')
    ...
    if args.mono:
        run_eval_keypoints_mono(
            ...
            args.type_e,
            cam_path=args.cam,
            step_gt=args.step_gt,
            mode=args.mode
        )
    else:
        run_eval_keypoints(
            args.path, args.out, args.type_i, args.type_o, args.step_gt, mode=args.mode, args=args
        )
