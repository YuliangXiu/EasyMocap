...
def openpose2shelf3D(pose3d, score):
    ...
    if True:
        ...
        shelf_pose[13] = shelf_pose[12] + (shelf_pose[13] -
                                           shelf_pose[12]) * np.array([0.75, 0.75, 1.5])
        shelf_pose[12] = shelf_pose[12] + (pose3d[0] -
                                           shelf_pose[12]) * np.array([1. / 2., 1. / 2., 1. / 2.])
...
def _readResult(filename, isA4d):
    ...
    if isA4d:
        ...
        for r in res_:
            r['keypoints3d'] = r['keypoints3d'][
                [4, 1, 5, 9, 13, 6, 10, 14, 0, 2, 7, 11, 3, 8, 12], :]
    return res_
...
def write_to_csv(filename, results, id_wise=True):
    ...
    print(
        tabulate.tabulate(contents, header, tablefmt='fancy_grid'),
...
def evaluate(actor3D, range_, out):
    ...
    names = [
        "Left Upper Arm", "Right Upper Arm", "Left Lower Arm", "Right Lower Arm", "Left Upper Leg",
        "Right Upper Leg", "Left Lower Leg", "Right Lower Leg", "Head", "Torso"
    ]
    ...
    for img_id in tqdm(range_):
        ...
        for res in result[img_id]:
            ests.append({
                'id': res['id'], 'keypoints3d': convert_openpose_shelf1(res['keypoints3d'])
            })
        gts = shelfgt[img_id]
        if len(gts) < 1:
            continue
