    ...
    if not skip:
        ...
        if os.name != 'nt':
            ...
            cmd = 'bin\\OpenPoseDemo.exe --image_dir {} --write_json {} --display 0'.format(
                join(os.getcwd(), image), join(os.getcwd(), keypoints)
            )
        if args.highres != 1:
...
def create_annot_file(annotname, imgname):
    ...
    annot = {
        'filename': filename, 'height': height, 'width': width, 'annots': [], 'isKeyframe': False
    }
...
def load_openpose(opname):
    mapname = {
        'face_keypoints_2d': 'face2d', 'hand_left_keypoints_2d': 'handl2d',
    ...
    for i, d in enumerate(data['people']):
        ...
        annot = {
            'bbox': bbox_from_openpose(keypoints), 'personID': pid + i, 'keypoints':
            keypoints.tolist(), 'isKeyframe': False
        }
...
def detect_frame(detector, img, pid=0):
    ...
    for i in range(len(lDetections)):
        annot = {
            'bbox': [float(d) for d in lDetections[i]['bbox']], 'personID': pid + i, 'keypoints':
            lDetections[i]['keypoints'].tolist(), 'isKeyframe': True
        }
        annots.append(annot)
    return annots
...
config_high = {
    ...
    'detect':
    {'MIN_PERSON_JOINTS': 10, 'MIN_BBOX_AREA': 5000, 'MIN_JOINTS_CONF': 0.3, 'MIN_BBOX_LEN': 150}
}
...
config_low = {
    ...
    {'nof_joints': 17, 'c': 48, 'checkpoint_path': 'data/models/pose_hrnet_w48_384x288.pth'},
    'detect':
    {'MIN_PERSON_JOINTS': 0, 'MIN_BBOX_AREA': 0, 'MIN_JOINTS_CONF': 0.0, 'MIN_BBOX_LEN': 0}
}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='openpose',
        choices=['openpose', 'yolo-hrnet'],
        help="model to extract joints from image"
    )
    parser.add_argument(
        '--ext', type=str, default='jpg', choices=['jpg', 'png'], help="image file extension"
    )
    parser.add_argument(
        '--annot',
        type=str,
    ...
    parser.add_argument('--openpose', type=str, default='/media/qing/Project/openpose')
    parser.add_argument('--render', action='store_true', help='use to render the openpose 2d')
    parser.add_argument('--no2d', action='store_true', help='only extract the images')
    parser.add_argument('--start', type=int, default=0, help='frame start')
    parser.add_argument('--end', type=int, default=10000, help='frame end')
    parser.add_argument('--step', type=int, default=1, help='frame step')
    parser.add_argument(
        '--low', action='store_true', help='decrease the threshold of human detector'
    )
    parser.add_argument(
        '--gtbbox',
        action='store_true',
    ...
    if os.path.isdir(args.path):
        ...
        if len(subs_videos) > len(subs_image):
            ...
            subs = []
            for video in videos:
                basename = extract_video(
                    video, args.path, start=args.start, end=args.end, step=args.step
                )
        ...
        if not args.no2d:
            for sub in subs:
                ...
                if mode == 'openpose':
                    extract_2d(
                        args.openpose, image_root, join(args.path, 'openpose', sub),
