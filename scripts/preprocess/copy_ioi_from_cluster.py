...
def copy_dataset(subject, src_path, tgt_path):
    ...
    if os.path.isdir(motion_dir):
        ...
        if len(apose_names) == 1:
            ...
            for outfit_seq_name in sorted(outfit_seq_names):
                tgt_outfit_name = [
                    outfit_name
                    for outfit_name in outfit_seq_name.split("_") if "outfit" in outfit_name
                ][0]
                src_outfit_dir = join(motion_dir, outfit_seq_name)
                tgt_outfit_dir = join(tgt_path, "fitting", person_id, tgt_outfit_name)

                if not os.path.exists(tgt_outfit_dir):
                    copy_all(src_outfit_dir, tgt_outfit_dir)

            all_front_paths = sorted(
                glob(join(tgt_path, "fitting", person_id, "*/images/07_C.jpg"))
            )
...
if __name__ == "__main__":
    ...
    subjects = [subject for subject in subjects if "03604" not in subject]
